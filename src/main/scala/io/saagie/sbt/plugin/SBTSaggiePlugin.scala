...
object SBTSaggiePlugin extends AutoPlugin {
    ...
    def create(jobSettings: JobSettings) = {
        ...
        try {
            ...
            val jobId = jobSettings.jobType match {
              ...
              case JobType.SPARK.value =>
                val body = "{\"platform_id\":\"" + jobSettings.platformId + "\",\"capsule_code\":\"" + jobSettings.jobType + "\",\"category\":\"" + jobSettings.jobCategory + "\",\"current\":{\"template\":\"spark-submit {driver_options} --class=" + jobSettings.mainClass + " {file} " + jobSettings.arguments + "\",\"file\":\"" + filename + "\",\"options\":{\"language_version\":\"" + jobSettings.sparkVersion + "\"},\"cpu\":" + jobSettings.cpu + ",\"memory\":" + jobSettings.mem + ",\"disk\":" + jobSettings.disk + ",\"releaseNote\":\"" + jobSettings.releaseNote + "\" },\"description\":\"" + jobSettings.description + "\",\"manual\":true,\"name\":\"" + jobSettings.jobName + "\"," + (if (jobSettings.streaming) "\"streaming\":true," else "") + "\"retry\":\"\",\"schedule\":\"R0/2016-07-06T15:47:52.051Z/P0Y0M1DT0H0M0S\"}"
                saagieClient.createJob(body)
  ...
  def update(jobSettings: JobSettings) = {
    val saagieClient = new SaagieClient(jobSettings)
    try {
    ...
    object autoImport {
        ...
        val javaVersion = settingKey[Int]("Represents the version of language you want to run your job.")
        ...
        lazy val commonSettings: Seq[Def.Setting[_]] = Seq(
            ...
            javaVersion in createSaagieJob := 8,
    ...
    override val projectSettings = createJobSettings ++ updateJobSettings
